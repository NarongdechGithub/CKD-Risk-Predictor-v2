...
def map_score_to_stage(score01: float) -> str:
    """
    แปลงความเสี่ยง 0..1 -> ระยะ CKD 1,2,3a,3b,4,5
    เกณฑ์ยึดขอบเดียวกับ level (0.2,0.4,0.6,0.8) และแบ่ง 3a/3b ที่ 0.5
    """
    s = float(score01)
    if s < 0.20: return "1"
    if s < 0.40: return "2"
    if s < 0.60:  # ภายในระยะ 3 แบ่งครึ่ง
        return "3a" if s < 0.50 else "3b"
    if s < 0.80: return "4"
    return "5"
...
def rule_level(payload: dict) -> Tuple[int, float]:
    """กฎ fallback: คืน (level, score01) เพื่อใช้ map เป็นระยะ CKD ได้"""
    def scale01(v, lo, hi):
        v = float(v)
        if hi <= lo: return 0.0
        v = max(min(v, hi), lo)
    ...
    MAX_SCORE = 7.5  # ผลรวมค่าน้ำหนักด้านบน
    score01 = max(0.0, min(score / MAX_SCORE, 1.0))
    print("Score from Rule (adjusted)", round(score01, 4), flush=True)

    level = map_score_to_level(score01)
    return level, score01
...
def predict():
    ...
    if MODEL_LOADED:
        ...
        try:
            ...
            level = map_score_to_level(score01)
            stage = map_score_to_stage(score01)
            print("Score from Model", round(score01, 4), flush=True)

        except Exception as e:
            level, score01 = rule_level(data)
            stage = map_score_to_stage(score01)
            return jsonify({
                "ok": True,
                "level": level,
    ...
    return jsonify({"ok": True, "level": level, "stage": stage, "message": messages[level]}), 200
